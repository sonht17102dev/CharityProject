...
public class ManagePartnerController extends BaseController{
	...
	public ModelAndView doEditPartner(HttpServletRequest request) {
		...
		Partner p = new Partner(Integer.parseInt(idS), name, slogan, logo, banner, introduce,
				Integer.parseInt(number_donateS), Integer.parseInt(total_moneyS), content, status);
		System.out.println(p.toString());
		p = partnerService.checkPartnerByNameExist(name);
		if (p == null) {
			try {
				...
				_mvShareAdmin.addObject("message", "Cập nhật thành công !!!");
				_mvShareAdmin.setViewName("redirect:quan-ly-doi-tac");
			} catch (Exception e) {
				e.printStackTrace();
			}

		} else {
			_mvShareAdmin.addObject("message", "Cập nhật thất bại - Tên Đối tác đã tồn tại vui lòng thử lại !!!");
			_mvShareAdmin.setViewName("admin/partner/editPartner");
