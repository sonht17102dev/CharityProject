...
public class AuthenticationSuccessHandlerImpl extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        if (roles.contains("ROLE_ADMIN")) {
            getRedirectStrategy().sendRedirect(request, response, "/admin/loginSuccess?usermail="+usermail);
//        	response.sendRedirect("/CharityApp/admin/loginSuccess?usermail="+usermail);
        } else if (roles.contains("ROLE_USER")) {
            getRedirectStrategy().sendRedirect(request, response, "/trang-chu/loginSuccess?usermail="+usermail);
